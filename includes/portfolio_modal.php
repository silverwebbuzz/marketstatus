...
function getPortEntry() {
    return parseFloat(document.getElementById('port-entry').value) || 0;
}
function getPortType() {
    return document.getElementById('port-type').value; // 'BUY' or 'SELL'
}

/*
 * For BUY:
 *   Target profit  → price > entry → pct positive  → green
 *   Stop loss risk → price < entry → pct negative   → red
 *
 * For SELL (short):
 *   Target profit  → price < entry → pct negative   → green (profit when price falls)
 *   Stop loss risk → price > entry → pct positive   → red
 *
 * % stored always as raw directional value (price - entry) / entry * 100
 * isProfit() decides colour based on trade type + field
 */
function isProfit(field, pct) {
    const isBuy = getPortType() === 'BUY';
    if (field === 'target') return isBuy ? pct > 0 : pct < 0;
    if (field === 'sl')     return isBuy ? pct < 0 : pct > 0; // SL is always a loss side
    return pct > 0;
}

function hintClass(field, pct) {
    // target: green = profit direction, red = wrong direction
    // sl:     always red (it's a loss)
    if (field === 'sl') return 'down';
    return isProfit(field, pct) ? 'up' : 'down';
}

function buildHint(field, price, pct) {
    const isBuy    = getPortType() === 'BUY';
    const sign     = pct > 0 ? '+' : '';
    const rrLabel  = field === 'target'
        ? (isProfit(field, pct) ? 'profit' : '⚠ wrong direction for ' + (isBuy ? 'BUY' : 'SELL'))
        : (isProfit(field, pct) ? '⚠ wrong direction for SL' : 'risk');
    return `₹${parseFloat(price).toFixed(2)}  (${sign}${pct.toFixed(2)}% — ${rrLabel})`;
}

// Price field changed → calculate %
function syncFromPrice(field) {
    const entry   = getPortEntry();
    ...
    if (entry > 0 && price > 0) {
        ...
        hint.textContent = buildHint(field, price, pct);
        hint.className   = 'pct-hint ' + hintClass(field, pct);
    } else {
        pctEl.value = ''; hint.textContent = ''; hint.className = 'pct-hint';
    }
}

// % field changed → calculate price
// User enters absolute % (positive = above entry, negative = below entry)
function syncFromPct(field) {
    const entry   = getPortEntry();
...
function resyncAllHints() {
    if (document.getElementById('port-target').value)         syncFromPrice('target');
    else if (document.getElementById('port-target-pct').value) syncFromPct('target');
    if (document.getElementById('port-sl').value)             syncFromPrice('sl');
    else if (document.getElementById('port-sl-pct').value)     syncFromPct('sl');
}

// Re-sync when entry or trade type changes
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('port-entry')?.addEventListener('input', resyncAllHints);
    document.getElementById('port-type')?.addEventListener('change', resyncAllHints);
});
</script>
